...
def helpMessage() {
    log.info"""
    Pipeline to map RNA-DNA interactions for MARGI protocol.

    Usage:
    The typical command for launching the pipeline:
      nextflow run margi.nf -profile test,conda -params-file params-margi.yml
...
workflow MARGI {
    ...
    TableStackChunksInput = TableChunked.map{ it -> [[sample:it[0].original_id, table_name:it[0].table_name], [it[0], it[1]]] }
                 ...
                 // Replace id with original_id and remove chunk from meta:
                    .map{ update_meta(it, [id: it[0].original_id+'.'+it[0].table_name]) }
                    .map{ removeKeys(it, ['chunk']) }.view()
