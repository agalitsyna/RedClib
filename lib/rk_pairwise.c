...
int main (int argc, char **argv)
{
    ...
    if ((argc!=8)&(argc!=9)) {
        ...
        fprintf(stderr, "Usage:\t%s query.bin library.bin entry_seq_len query_seq_len min_shift_left min_shift_right allowed_mismatches (report_single_hit) > output.tsv\n\n", argv[0]);
    ...
    unsigned int entry_seq_len, query_seq_len, allowed_mis, add_mis, report_single_hit;
    ...
    if (argc==9) {
        sscanf(argv[8],"%u",&report_single_hit);
    } else {
        report_single_hit = 1;
    }
...
//
    ...
    unsigned int query_length = 3+(query_seq_len-1)/8;
    ...
    unsigned int buf[entry_length+1], buf_query[entry_length+1], * array_shift_l[10], * array_shift_r[10];
    unsigned int c, i, j;
    int is_hit;
    ...
    {
        fread(buf_query, 4, query_length, query_bin); //
        is_found = 0;

        oligo_len = query_length; // in bins
