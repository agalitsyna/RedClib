...
process RKLIB_CHECK_COMPLEMENTARY {
    ...
    def rna_compl_length = meta_compl.length
    def extraN = "N"*(500+3*rna_compl_length)

    def left_oligo_length = meta_oligos_left.expected_length
    def right_oligo_length = meta_oligos_right.expected_length
    ...
    paste <(awk '{print \$1, ${table_cols_left}}' ${table} | tail -n +2) \\
          <(head -n -1 ${aligned_left} | tail -n +2 | awk '{print \$4}') \\
    ...
    paste <(awk '{print \$1, ${table_cols_right}}' ${table} | tail -n +2) \\
          <(head -n -1 ${aligned_right} | tail -n +2 | awk '{print \$4}') \\
    ...
    # Align compl regions to each library:
    rk_pairwise ${prefix}.rna-end.2.revcomp.bin ${bin_reads[0]} ${rna_compl_length} ${read_length} \\
                   0 ${read_length-rna_compl_length} 0 > ${prefix}.hits_compl.R1.tsv

    rk_pairwise ${prefix}.rna-end.1.revcomp.bin ${bin_reads[0]} ${rna_compl_length} ${read_length} \\
               0 ${read_length-rna_compl_length} 0 > ${prefix}.hits_compl.R2.tsv
