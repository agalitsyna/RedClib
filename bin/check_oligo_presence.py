        ...
        with open(file_oligo_hits, "r") as hits_f:
            ...
            while len(fastq_table_line) > 0:
                ...
                if orientation == "F":
                    ...
                    raise ValueError(
                        f"Orientation '{orientation}' is not supported. Please, specify 'F' or 'R' instead"
                    )

                oligo_position_start = int(
                    hits_table_line.split()[3]
                )  # Start position of the oligo in the read
                idx = hits_table_line.split()[
                    0
                ]  # Number of the read in the table with hits
                if oligo_position_start > len(read):
                    ret = 0
                else:
                    if (
                        read[
                            oligo_position_start
                            + position_in_oligo : oligo_position_start
                            + position_in_oligo
                            + len(oligo)
                        ]
                        == oligo
