...
if len(argv)<4:
    print("""Add columns to parquet file. Column schema should be tab-separated and contain three columns: columns_name, column_format and column_expression.
...
with open(columns_schema, 'r') as input_file:
    for line in input_file.readlines():
        ...
        assert np.all([x not in column_name for x in prohibited_symbols]), "Check the column name. It cannot contain " + ",".join(prohibited_symbols)
                ...
                if (node.id not in list(vars().keys())) and (node.id not in dir(__builtins__)):
                    try:
                        ...
                        try:
                            ...
                            raise ValueError(f"Variable {node.id} is not available from input/created pyarrow file. "
...
if n_evaluated>0:
    print(f"Evaluated {n_evaluated} expressions, including columns: {', '.join(arrays_dict.keys())}")
