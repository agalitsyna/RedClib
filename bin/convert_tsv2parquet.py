...
if len(argv)!=4 and len(argv)!=5:
    print("Convert tsv table to parquet. May add suffix to colnames (should not contain dots), will be added after '__'. Usage: tsv2parquet.py csv_file parquet_file chunksize (suffix, optional)")
    exit()
...
for i, chunk in enumerate(csv_stream):
    if i == 0:
        columns = {x: x.replace('#', '')+('__'+suffix if len(suffix)>0 else '') for x in chunk.columns}
